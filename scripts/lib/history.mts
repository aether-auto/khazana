 ...
 * so the prune step (scripts/prune-history.mts) knows the AGE of every Read,
 * feed item and audio file without rearchitecting the single-file feed.
 *
 ...
 *
 * This module is additive: the future P9 generate/build step calls
 * `appendBuildDay()` after it writes that day's Reads + audio; the prune step
 * calls `readLedger()`. Nothing in the live ingest/curate pipeline depends on
 * it, so it can be wired in (or not) without touching verified behavior. If the
 * ledger is ever missing or corrupt, `readLedger()` returns an empty ledger and
 * the prune falls back to scanning frontmatter — it never throws.
 */
...
export interface BuildDay {
  ...
  feedItemIds: string[];
  /** Narration audio files emitted that day, as repo-relative paths under apps/site/public. */
  audioFiles: string[];
}
 ...
 * duplicating it. The future P9 build step calls this once, after it has
 * written the day's Reads + rendered audio.
 */
