# AGENTS.md — khazana cofounder memory
...
| **Codex Sonnet 5** (subscription) | ALL blog copywriting, every format including Chronicle (survey → write → verify); Source Scout credibility reasoning; the fact-check/verification passes | subscription, default tier |
| **Codex Opus** (subscription) | The Reads-run **orchestrator's own** curation/QC judgement only (`reads-run` picking the slate, judging verdicts) — never per-format prose. Used sparingly. | subscription, used rarely |
