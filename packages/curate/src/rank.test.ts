...
import { hasFullText, rankItems, applyDiversityFloor, W_MEDIA, FEATURED_SIZE, DIVERSITY_WINDOW, DIVERSITY_MIN_VIDEO, DIVERSITY_MIN_AUDIO } from "./rank.js";
...
test("a video item WITH a transcript receives full-text credit, not media credit", () => {
  ...
  // The score delta should equal W_FULLTEXT - W_MEDIA, not double-counted.
  const withScore = ranked[0]!.tasteScore!;
  const withoutScore = ranked[1]!.tasteScore!;
  expect(withScore - withoutScore).toBeCloseTo(1.5 - W_MEDIA, 5);
});
